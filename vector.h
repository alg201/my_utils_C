 ...
 * Generates a dynamic vector (like std::vector from C++) which type is
 * indicated in `type` parameter
 *
 * Generates:
 *  - typedef struct {
 *      type *data;
 ...
 *      - Creates an empty vector. vector has originally a capacity of 4 elements.
 *    vector_type *resize_vector_type(vector_type *v, size_t bytes);
 *      - Resizes the vector adding the bytes that recives as second argument.
 * If vector.data points to nullptr, the allocates recived bytes on memory.
 ...
 *      - Locates and returns the index of the item. In case the item couldn't
 * be found, returns INDEX_NOT_FOUND.
 *    vector_type *remove_index_type(vector_type *v, size_t index);
 *      - Removes the item given the index, then shrinks the vector. In case the
 * index is not found, the function does nothing 
 *    vector_type *remove_item_type(vector_type *v, type index);
 *      - Removes the item on the vector. In case this item couldn't be found,
 * the function does nothing 
 *    void destroy_vector_type(vector_type *v);
 *      - Destroys the vector and frees the memory. This doesn't free the memory
 * of the data inside. (If the vector is storing pointers, the memory of the
 * data which is pointed by this, isn't free, should be done manually) void
 */
...
typedef struct {                                                               \
  ...
  vector_##type *create_vector_##type() {                                      \
    ...
    v->data = malloc(4 * sizeof(type));                                    \
    v->len = 0;                                                                \
    v->capacity = 4;                                                           \
    return v;                                                                  \
  }                                                                            \
                                                                               \
  static vector_##type *resize_vector_##type(vector_##type *v, size_t bytes) { \
    if (v->data == NULL) {                                                     \
      v->data = malloc(bytes);                                                 \
      return v;                                                                \
    }                                                                          \
  ...
  vector_##type *append_##type(vector_##type *v, type val) {                   \
    if (v->len == v->capacity) {                                               \
      v->capacity *= 2;                                                        \
      v = resize_vector_##type(v, (v->capacity / 2) * sizeof(type));           \
    }                                                                          \
    v->data[v->len] = val;                                                     \
    v->len++;                                                                  \
                                                                               \
